...
export default function TeacherView({ auth }: TeacherViewProps) {
    ...
    const validatePrompt = () => {
        ...
        if (promptType === 'MC' && mcOptions.filter((o) => o.trim().length > 0).length < 2)
            return false;
        return true;
    };
    ...
    const handleSendPrompt = () => {
        if (!validatePrompt())
            return showModal(
                'error',
                'Invalid Prompt',
                'Please enter text and at least 2 options for MC.'
            );
    ...
    const endSession = () => {
        showModal(
            'confirm',
            'End Session',
            'End the session? All students will be disconnected.',
            () => {
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div className="flex-1 overflow-hidden">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span
                                                    className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded text-white ${promptType === 'MC' ? 'bg-purple-500' : promptType === 'SCALE' ? 'bg-orange-500' : 'bg-blue-500'}`}
                                                >
                                                    {promptType}
                                                </span>
                                            ...
                                            {promptType === 'MC' && (
                                                <p className="text-xs text-gray-500">
                                                    {mcOptions.filter((o) => o).length} Options
                                                </p>
                                            )}
                                        </div>
                                        <button
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 h-fit">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            {previousSessions.length > 0 && (
                                ...
                                >
                                    {showPreviousSessions ? 'Hide' : 'Show'} (
        ...
        <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 pb-20 relative px-4 sm:px-0">
            ...
            <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="flex items-center space-x-4 w-full sm:w-auto">
                    ...
                    <div>
                        <h2 className="text-xs text-gray-500 font-bold uppercase tracking-wide">
                            Room Code
                        </h2>
                        <div
                            ...
                            onClick={copyCode}
                        >
                            <span className="text-3xl sm:text-4xl font-mono font-bold text-gray-900">
                                {joinCode}
                            </span>
                ...
                <div className="flex items-center justify-between w-full sm:w-auto sm:space-x-6">
                    ...
                    <button
                        onClick={endSession}
                        className="ml-4 px-3 sm:px-4 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition flex items-center text-sm font-bold"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div
                        className={`p-4 sm:p-6 rounded-xl shadow-lg border-t-4 transition duration-300 ${submissionCount > 0 && !swapComplete ? 'bg-white border-green-500 opacity-100' : 'bg-gray-50 border-gray-300 opacity-80'}`}
                    >
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        socket.emit('UPDATE_SESSION_SETTINGS', {
                                            joinCode,
                                            maxSwapRequests: parseInt(e.target.value),
                                        });
                                    }}
                        ...
                        <p className="text-gray-600 mb-6 text-sm sm:text-base">
                            Once enough students have submitted, initiate the swap.
                        </p>
                        <button
                            onClick={() => socket.emit('TRIGGER_SWAP', { joinCode })}
                            disabled={submissionCount < 2 || swapComplete}
                            className={`w-full py-3 sm:py-4 text-white font-bold rounded-xl text-lg shadow-md transition flex items-center justify-center ${submissionCount >= 2 && !swapComplete ? 'bg-green-600 hover:bg-green-700 transform hover:scale-[1.02]' : 'bg-gray-300 cursor-not-allowed'}`}
                        >
                            ...
                            <div className="max-h-60 overflow-y-auto space-y-2 border border-gray-100 rounded-lg p-2 bg-gray-50">
                                {liveThoughts.length === 0 ? (
                                    ...
                                    liveThoughts.map((t) => (
                                        <div
                                            key={t.id}
                                            className="bg-white p-3 rounded-md shadow-sm border border-gray-200 flex justify-between items-start group"
                                        >
                                            <div>
                                            ...
                                            <button
                                                onClick={() =>
                                                    showModal(
                                                        ...
                                                        'Delete this thought?',
                                                        () =>
                                                            socket.emit('TEACHER_DELETE_THOUGHT', {
                                                                joinCode,
                                                                thoughtId: t.id,
                                                            }),
                                                        undefined,
                                                ...
                                                className="text-red-300 hover:text-red-500 p-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition"
                                            >
                                                <Trash2 className="w-4 h-4" />
                                            </button>
                                        </div>
                ...
                <div className="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col h-[400px] lg:h-[600px] border border-gray-200">
                    <div className="p-4 bg-gray-50 border-b border-gray-200">
                        <h3 className="font-bold text-gray-700 flex items-center">
                            <Users className="w-5 h-5 mr-2 text-gray-500" /> Class Roster
                        </h3>
                    </div>
