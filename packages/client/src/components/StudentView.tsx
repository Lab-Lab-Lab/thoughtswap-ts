...
export default function StudentView({ joinCode, auth, onJoin }: StudentViewProps) {
    ...
    const renderContent = () => {
        switch (status) {
            case 'IDLE':
                return (
                    <div className="p-6 sm:p-8 bg-white rounded-xl shadow-lg max-w-sm w-full">
                        <h3 className="text-xl sm:text-2xl font-bold mb-4 text-gray-800">Join a Course</h3>
                        <p className="text-gray-600 mb-6 text-sm sm:text-base">Enter the 6-character room code from your teacher.</p>
                        <div className='space-y-4'>
                            <input type="text" placeholder="Room Code" value={inputCode} onChange={(e) => setInputCode(e.target.value.toUpperCase())} className="w-full px-4 py-2 border border-gray-300 rounded-lg text-center text-xl font-mono tracking-widest uppercase focus:ring-indigo-500 focus:border-indigo-500" maxLength={6} />
                            <button onClick={handleJoinClick} disabled={inputCode.length !== 6} className="w-full flex items-center justify-center px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white font-semibold rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            case 'JOINED':
                return (
                    <div className="flex flex-col items-center justify-center p-10 bg-white rounded-xl shadow-lg w-full max-w-sm">
                        <Loader2 className="w-8 h-8 text-indigo-500 animate-spin mb-4" />
                        <h3 className="text-xl font-semibold text-gray-700">Awaiting Prompt...</h3>
                        <p className="text-gray-500 text-center">The teacher will send the prompt shortly.</p>
                        {!notificationsEnabled && <button onClick={requestNotificationPermission} className="mt-4 text-xs text-indigo-500 flex items-center hover:underline"><Bell className="w-3 h-3 mr-1" /> Enable Notifications</button>}
                    </div>
                );
            case 'ANSWERING':
                return (
                    <div className="bg-white p-4 sm:p-10 rounded-xl shadow-lg w-full max-w-2xl">
                        <div className="flex items-center text-indigo-600 mb-4">
                            <MessageSquare className="w-6 h-6 mr-3" />
                            <h3 className="text-lg sm:text-2xl font-bold">Current Prompt</h3>
                        </div>
                        <blockquote className="border-l-4 border-indigo-400 pl-4 py-2 mb-6 italic text-gray-700 text-base sm:text-lg">{prompt}</blockquote>
                        ...
                        <button onClick={() => { socket.emit('SUBMIT_THOUGHT', { joinCode: inputCode, content: responseInput, promptUseId }); setStatus('SUBMITTED'); }} disabled={!responseInput.trim()} className="w-full px-4 py-3 bg-green-500 hover:bg-green-600 text-white font-bold rounded-lg transition mt-4 shadow-md disabled:opacity-50">Submit Response</button>
                    </div>
                );
            case 'SUBMITTED':
                return (
                    <div className="flex flex-col items-center p-8 sm:p-10 bg-indigo-50 rounded-xl shadow-xl w-full max-w-md text-center border-2 border-indigo-400">
            ...
            case 'DISCUSSING':
                return (
                    <div className="bg-indigo-100 p-4 sm:p-10 rounded-xl shadow-2xl w-full max-w-2xl border-4 border-indigo-600">
                        <div className="flex items-center text-indigo-800 mb-4">
                            <RotateCcw className="w-6 h-6 sm:w-8 sm:h-8 mr-3" />
                            <h3 className="text-xl sm:text-3xl font-extrabold">Peer Review Time!</h3>
                        </div>
                        <p className="text-base sm:text-lg text-gray-700 mb-6">Discuss this anonymous peer's response:</p>
                        <blockquote className="bg-white p-4 sm:p-6 rounded-lg border-l-8 border-yellow-500 italic text-lg sm:text-xl shadow-inner text-gray-800 mb-6">"{swappedThought}"</blockquote>
                        <div className="flex justify-end">
                            <button onClick={() => socket.emit('STUDENT_REQUEST_NEW_THOUGHT', { joinCode: inputCode, currentThoughtContent: swappedThought })} className="text-sm text-indigo-600 hover:text-indigo-800 underline flex items-center font-semibold">
